...
RUN wget https://github.com/zippiehq/cartesi-lambada-guest-tools/releases/download/v0.15.0.4/machine-emulator-tools-v0.15.0.deb && mv machine-emulator-tools-v0.15.0.deb /tool-image/
...
RUN cartesi-machine --skip-root-hash-check --append-bootargs="no4lvl loglevel=8 init=/usr/sbin/lambada-preinit systemd.unified_cgroup_hierarchy=0 rootfstype=ext4 ro CARTESI_TMPFS_SIZE=1G systemd.journald.forward_to_console=1" \
     --ram-image=./artifacts/linux.bin --flash-drive="label:root,filename:/clean-image.ext2" --ram-length=2Gi --store=/lambada-base-machine-presparse \
     --max-mcycle=0 && \
